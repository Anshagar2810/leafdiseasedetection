...
with col2:
    ...
    if test_image:
        if st.button("🚀 INITIATE SCAN"):
            ...
            st.balloons()
            st.markdown(f"""
            <div class="result-container" style="background: rgba(46, 125, 50, 0.1); padding: 20px; border-radius: 20px; border: 1px solid #2e7d32; margin-top: 20px;">
                <h3 style="color: #1b5e20; margin: 0;">Diagnostic Result:</h3>
                <h2 style="color: #2e7d32; font-weight: 800; font-size: 2.2rem;">{CLASS_NAMES[result_index].replace('___', ' - ')}</h2>
            ...
            """, unsafe_allow_html=True)
            st.info("💡 **Recommended Action:** Remove infected leaves and apply organic fungicide if applicable.")
    else:
